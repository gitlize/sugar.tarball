...
public class LinearLeLiteral extends LinearLiteral {
	 ...
	 * 
	 */
	private void encode(OldEncoder encoder, IntegerVariable[] vs, int i, int s, int[] clause)
	throws IOException {
		if (i >= vs.length - 1) {
			...
			if (a >= 0) {
				...
				// XXX
				if (true) {
					clause[i] = OldEncoder.negateCode(vs[i].getCodeLE(lb - 1));
					encode(encoder, vs, i+1, s+a*(lb-1), clause);
					Iterator<Integer> iter = domain.values(lb, ub); 
					while (iter.hasNext()) {
						int c = iter.next();
						// vs[i]<=c -> ...
						clause[i] = OldEncoder.negateCode(vs[i].getCodeLE(c));
						encode(encoder, vs, i+1, s+a*c, clause);
					}
				} else {
					clause[i] = OldEncoder.negateCode(vs[i].getCodeLE(lb - 1));
